...
3. Kafka 的 consumer 工作流程？
   ...
   3. consumer 将自己保存的 offset 发送给Leader
...
15. **leader 挂了**会怎样？（leader failover）

    当Leader挂了之后，controller 默认会从 ISR 中选择一个 replica 作为 Leader 继续工作，条件是新 Leader 必须有挂掉 Leader 的所有数据。
...
16. **broker 挂了**会怎样？（broker failover）
...
17. **controller 挂了**会怎样？（controller failover）
